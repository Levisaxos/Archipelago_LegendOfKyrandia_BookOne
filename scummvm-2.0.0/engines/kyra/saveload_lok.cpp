...
#include "ap_bridge.h"   // Archipelago: tag saves with the server/slot

namespace Kyra {

// Magic tag for the Archipelago metadata chunk appended to LoK saves (fork extension).
#define KYRA_AP_SAVE_TAG MKTAG('A', 'P', 'K', '1')

static Common::String apStripAddressPrefix(const Common::String &name) {
	// Remove a leading "host:port | " decoration so re-saving doesn't stack it.
	const char *sep = strstr(name.c_str(), " | ");
	if (sep) {
		Common::String prefix(name.c_str(), (uint32)(sep - name.c_str()));
		if (prefix.contains(':'))   // looks like host:port -> it's our decoration
			return Common::String(sep + 3);
	}
	return name;
}
...
Common::Error KyraEngine_LoK::loadGameState(int slot) {
	...
	// Archipelago metadata chunk (fork extension), if present. Guard on remaining
	// bytes so vanilla saves (no chunk) don't trip the eof check below.
	if (in->pos() + 4 <= in->size()) {
		uint32 apTag = in->readUint32BE();
		if (apTag == KYRA_AP_SAVE_TAG) {
			char buf[256];
			uint16 hl = in->readUint16BE();
			if (hl > 255)
				hl = 255;
			in->read(buf, hl);
			buf[hl] = 0;
			ConfMan.set("ap_host", buf);
			uint16 sl = in->readUint16BE();
			if (sl > 255)
				sl = 255;
			in->read(buf, sl);
			buf[sl] = 0;
			ConfMan.set("ap_slot", buf);
		}
	}
...
Common::Error KyraEngine_LoK::saveGameStateIntern(int slot, const char *saveName, const Graphics::Surface *thumb) {
	...
	// Archipelago: prefix the visible save name with the server address so the
	// player can tell which save belongs to which seed when juggling several.
	Common::String apHost = ConfMan.hasKey("ap_host") ? ConfMan.get("ap_host") : Common::String();
	Common::String apSlot = ConfMan.hasKey("ap_slot") ? ConfMan.get("ap_slot") : Common::String();
	if (apHost.hasPrefix("ws://"))
		apHost = Common::String(apHost.c_str() + 5);
	else if (apHost.hasPrefix("wss://"))
		apHost = Common::String(apHost.c_str() + 6);

	Common::String desc = apStripAddressPrefix(saveName);
	if (KyraAP::isConnected() && !apHost.empty())
		desc = apHost + " | " + desc;

	Common::OutSaveFile *out = openSaveForWriting(fileName, desc.c_str(), thumb);
	if (!out)
		return _saveFileMan->getError();
	...
	// Archipelago metadata chunk (fork extension): the server/slot this save
	// belongs to, so it stays linked to its seed across save/load.
	out->writeUint32BE(KYRA_AP_SAVE_TAG);
	out->writeUint16BE((uint16)apHost.size());
	if (apHost.size())
		out->write(apHost.c_str(), apHost.size());
	out->writeUint16BE((uint16)apSlot.size());
	if (apSlot.size())
		out->write(apSlot.c_str(), apSlot.size());
