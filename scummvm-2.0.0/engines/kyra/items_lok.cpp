...
void KyraEngine_LoK::setHandItem(Item item) {
	setMouseItem(item);
	_itemInHand = item;
	// --- AP capture: every item that enters the player's hand, from ANY source
	//     (ground pickup, scripted catch like the teardrop, plucked rose, container,
	//     NPC give). APSCRAPE only sees floor items; this catches the rest. Capture
	//     stderr / ScummVM log and walk the game once to build the acquisition map.
	//     Prefix "APGET". ---
	if (item != kItemNone) {
		const char *nm = _itemList ? _itemList[getItemListIndex(item)] : "?";
		int sc = _currentCharacter ? _currentCharacter->sceneId : -1;
		warning("APGET scene=%d item=%d itemname=%s source=hand", sc, (int)item, nm);
	}
}
