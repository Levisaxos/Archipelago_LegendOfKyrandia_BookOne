...
void Animator_LoK::prepDrawAllObjects() {
	...
	while (curObject) {
		// AP dev: skip the actor objects (Brandon + NPCs, indices 0-4) when the
		// character-free capture flag is set; items/scene anims (index >= 5) still draw.
		if (curObject->active && !(_noDrawCharactersFlag && curObject->index <= 4)) {
