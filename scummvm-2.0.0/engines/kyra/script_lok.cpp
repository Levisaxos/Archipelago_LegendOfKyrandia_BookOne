...
int KyraEngine_LoK::o1_customPrintTalkString(EMCState *script) {
	// --- AP capture: NPC / talking-tree dialogue. NPCs (Herman, the willow tree,
	//     etc.) speak through THIS opcode, NOT characterSays — positioned, colour-
	//     coded text via printTalkTextMessage. The colour identifies the speaker;
	//     voc is the voice-file id (talkie builds). Prefix "APTALK". ---
	{
		bool talkie = _flags.isTalkie;
		const char *txt = talkie ? stackPosString(1) : stackPosString(0);
		int color = talkie ? (stackPos(4) & 0xFF) : (stackPos(3) & 0xFF);
		int voc = talkie ? stackPos(0) : -1;
		warning("APTALK scene=%d color=%d voc=%d text=\"%s\"",
		        _currentCharacter ? _currentCharacter->sceneId : -1,
		        color, voc, txt ? txt : "");
	}
	if (_flags.isTalkie) {
...
int KyraEngine_LoK::o1_printText(EMCState *script) {
	...
	// --- AP capture: scripted positioned text. NPC scene-script dialogue (the
	//     talking willow tree, Herman, etc.) comes through here rather than
	//     characterSays — there's no character entity, so the text COLOR identifies
	//     the speaker and (x,y) is where it's drawn. Also fires for signs/labels;
	//     filter by colour/content. Prefix "APTEXT". ---
	warning("APTEXT scene=%d x=%d y=%d color=%d text=\"%s\"",
	        _currentCharacter ? _currentCharacter->sceneId : -1,
	        stackPos(1), stackPos(2), stackPos(3),
	        stackPosString(0) ? stackPosString(0) : "");
