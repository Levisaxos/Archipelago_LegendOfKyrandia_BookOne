...
#include "ap_bridge.h"     // Archipelago client bridge (plain interface; impl in ap_bridge.lib)
#include "ap_item_map.h"   // generated: AP item id -> engine item id
...
void KyraEngine_LoK::mainLoop() {
	...
	// Archipelago: in-game connection form (shown once, before play begins).
	apConnectionScreen();
	...
	while (!shouldQuit()) {
		...
		updateMousePointer();
		_timer->update();
		KyraAP::poll();   // Archipelago: pump the client once per frame
		// Deliver any items the server sent us onto the floor near Brandon.
		{
			long long apItem;
			while (KyraAP::nextReceivedItem(&apItem)) {
				int eng = apItemToEngineId(apItem);
				if (eng >= 0 && eng <= 106)
					apDropReceivedItem(eng);
			}
		}
		_sound->process();
		updateTextFade();
...
void KyraEngine_LoK::apConnectionScreen() {
	if (KyraAP::isConnected())
		return;   // already connected (e.g. mainLoop re-entered after a load)

	char host[64], slot[40], pass[40];
	Common::strlcpy(host, ConfMan.hasKey("ap_host") ? ConfMan.get("ap_host").c_str() : "ws://localhost:38281", sizeof(host));
	Common::strlcpy(slot, ConfMan.hasKey("ap_slot") ? ConfMan.get("ap_slot").c_str() : "Player1", sizeof(slot));
	Common::strlcpy(pass, ConfMan.hasKey("ap_password") ? ConfMan.get("ap_password").c_str() : "", sizeof(pass));

	char *fields[3] = { host, slot, pass };
	const int caps[3] = { (int)sizeof(host) - 1, (int)sizeof(slot) - 1, (int)sizeof(pass) - 1 };
	const char *labels[3] = { "Server", "Slot", "Password" };
	int active = 0;

	// Make palette slot 255 white so the 0xFF text colour is visible.
	_screen->getPalette(0)[765] = 0x3F;
	_screen->getPalette(0)[766] = 0x3F;
	_screen->getPalette(0)[767] = 0x3F;
	_screen->setScreenPalette(_screen->getPalette(0));

	Common::EventManager *em = _system->getEventManager();
	bool confirm = false, skipped = false;
	while (!confirm && !shouldQuit()) {
		Common::Event ev;
		while (em->pollEvent(ev)) {
			...
			if (ev.type != Common::EVENT_KEYDOWN)
				continue;
			Common::KeyCode kc = ev.kbd.keycode;
			char ascii = (char)ev.kbd.ascii;
			if (kc == Common::KEYCODE_ESCAPE) {
				skipped = true;   // play offline (no AP) — also handy for scrape runs
				confirm = true;
			} else if (kc == Common::KEYCODE_RETURN || kc == Common::KEYCODE_KP_ENTER) {
				confirm = true;
			} else if (kc == Common::KEYCODE_TAB || kc == Common::KEYCODE_DOWN) {
				active = (active + 1) % 3;
			} else if (kc == Common::KEYCODE_UP) {
				active = (active + 2) % 3;
			} else if (kc == Common::KEYCODE_BACKSPACE || kc == Common::KEYCODE_DELETE) {
				int l = strlen(fields[active]);
				if (l > 0)
					fields[active][l - 1] = 0;
			} else if ((uint8)ascii > 31 && (uint8)ascii < 127) {
				int l = strlen(fields[active]);
				if (l < caps[active]) {
					fields[active][l] = ascii;
					fields[active][l + 1] = 0;
				}
			}
		}

		_screen->fillRect(16, 28, 304, 132, 12);
		_text->printText("ARCHIPELAGO  -  CONNECT", 24, 34, 0xFF, 12, 0);
		for (int i = 0; i < 3; ++i) {
			int y = 58 + i * 20;
			_text->printText(labels[i], 24, y, 0xFF, 12, 0);
			char shown[80];
			if (i == 2) {
				int l = strlen(fields[2]), k = 0;
				for (; k < l && k < 60; ++k)
					shown[k] = '*';
				shown[k] = 0;
			} else {
				Common::strlcpy(shown, fields[i], sizeof(shown));
			}
			if (i == active)
				Common::strlcat(shown, "_", sizeof(shown));
			_text->printText(shown, 96, y, 0xFF, 12, 0);
		}
		_text->printText("TAB = field   ENTER = connect   ESC = skip", 24, 126, 0xFF, 12, 0);
		_screen->updateScreen();
		_system->delayMillis(15);   // raw sleep: do NOT use delay() here, it drains our input events
	}
	...
	if (!skipped) {
		// Remember the values and begin connecting.
		ConfMan.set("ap_host", host);
		ConfMan.set("ap_slot", slot);
		ConfMan.set("ap_password", pass);
		ConfMan.flushToDisk();
		KyraAP::init(host, slot, pass);

		// Show status until connected or a 15s timeout; mainLoop keeps polling after.
		uint32 deadline = _system->getMillis() + 15000;
		while (!shouldQuit() && !KyraAP::isConnected() && _system->getMillis() < deadline) {
			KyraAP::poll();
			_screen->fillRect(16, 28, 304, 132, 12);
			_text->printText("ARCHIPELAGO", 24, 50, 0xFF, 12, 0);
			_text->printText(KyraAP::statusText(), 24, 70, 0xFF, 12, 0);
			_screen->updateScreen();
			delay(30);
		}
		if (KyraAP::isConnected()) {
			_screen->fillRect(16, 28, 304, 132, 12);
			_text->printText("Connected - entering Kyrandia", 24, 60, 0xFF, 12, 0);
			_screen->updateScreen();
			delay(900);
		}
	}

	// Wipe the connection panel and restore the game scene underneath it (else the
	// box lingers until the dirty-rect system happens to repaint that area).
...
void KyraEngine_LoK::apDropReceivedItem(int engineItem) {
	// Drop an item received from Archipelago onto the floor near Brandon, so it
	// never has to fight for one of the 12 inventory slots.
	assert(_currentCharacter->sceneId < _roomTableSize);
	Room *room = &_roomTable[_currentCharacter->sceneId];

	int freeSlot = -1;
	for (int s = 0; s < 12; ++s) {
		if (room->itemsTable[s] == kItemNone) {
			freeSlot = s;
			break;
		}
	}
	if (freeSlot < 0) {
		// Floor is full (12 items). For now we note it; a queue/retry is a TODO.
		drawSentenceCommand("Received an item, but the floor is full!", 6);
		return;
	}

	int dx = _currentCharacter->x1;
	int dy = _currentCharacter->y1;
	if (dy < 24)
		dy = 24;
	if (dy > 130)
		dy = 130;
	int dropFromY = (dy > 56) ? dy - 48 : 16;

	itemDropDown(dx, dropFromY, dx, dy, (byte)freeSlot, engineItem);
	itemSpecialFX(dx, dy, engineItem);

	Common::String msg = Common::String("Received ") + _itemList[getItemListIndex(engineItem)];
	drawSentenceCommand(msg.c_str(), 6);
}
...
int KyraEngine_LoK::processInputHelper(int xpos, int ypos) {
	...
	if (item != 0xFF) {
		if (_itemInHand == kItemNone) {
			...
			//     (sceneId, slot) -> location map comes from Phase 1 instrumentation.) ---
			long long apLocId = -1;
			if (item2 == 0)   // Garnet (engine id 0) = "Brandon's Home - Garnet (tree house)"
				apLocId = 1010002;

			if (apLocId >= 0 && KyraAP::isConnected()) {
				// Suppress the vanilla grant: the item vanishes and the check is
				// sent to Archipelago. The server sends back whatever was placed
				// here; mainLoop drops it on the floor.
				_itemInHand = item2;
				magicOutMouseItem(2, -1);   // poof
				KyraAP::sendCheck(apLocId);
				Common::String msg = Common::String("Sent ") + _itemList[getItemListIndex(item2)];
				drawSentenceCommand(msg.c_str(), 6);   // bottom bar, auto-fades
				clickEventHandler2();
				return 1;
			}
			// --- end Archipelago send ---
