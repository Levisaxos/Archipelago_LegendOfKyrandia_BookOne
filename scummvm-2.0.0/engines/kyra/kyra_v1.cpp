...
// AP: human-readable names for known game flags (see research/flags.md). Returns
// "" for unknown flags. External linkage so the `flags` console dump can share it.
const char *kyraApFlagName(int flag) {
	switch (flag) {
	case 7:   return "AMULET_OBTAINED";
	case 9:   return "BRIDGE_CROSSED";
	case 42:  return "ALTAR_MARBLE_PLACED";
	case 45:  return "AMULET_AT_ALTAR";
	case 59:  return "BRIDGE_QUEST_STARTED";
	case 63:  return "BRIDGE_SAW_GIVEN";
	case 65:  return "MERITH_CHASE_ACTIVE";
	case 124: return "BRYNN_NOTE_SHOWN";
	case 129: return "BRYNN_NOTE_DECODED";
	case 130: return "BRYNN_CLUE_HINT";
	case 138: return "WILLOW_HEALED";
	case 242: return "NOTE_PICKED_UP";
	default:  return "";
	}
}
...
int KyraEngine_v1::setGameFlag(int flag) {
	...
	// --- AP capture: game-flag SET, logged only on the real 0->1 transition (the
	//     actual event, not idempotent re-sets). The flag id is the event-location
	//     key (e.g. "bridge repaired"). Correlate with surrounding APSCENE/APTALK by
	//     timestamp. Prefix "APFLAG"; name from kyraApFlagName() when known. ---
	if (!((_flagsTable[flag >> 3] >> (flag & 7)) & 1))
		warning("APFLAG set flag=%d (0x%X) %s", flag, flag, kyraApFlagName(flag));
...
int KyraEngine_v1::resetGameFlag(int flag) {
	assert((flag >> 3) >= 0 && (flag >> 3) <= ARRAYSIZE(_flagsTable));
	// --- AP capture: game-flag RESET, logged only on the real 1->0 transition.
	//     Prefix "APFLAG"; name from kyraApFlagName() when known. ---
	if ((_flagsTable[flag >> 3] >> (flag & 7)) & 1)
		warning("APFLAG reset flag=%d (0x%X) %s", flag, flag, kyraApFlagName(flag));
	_flagsTable[flag >> 3] &= ~(1 << (flag & 7));
	return 0;
}
