...
void KyraEngine_LoK::characterSays(int vocFile, const char *chatStr, int8 charNum, int8 chatDuration) {
	...
	// --- AP capture: every spoken line. speaker = who is talking (0 = Brandon,
	//     1-4 main chars, 5-10 NPCs); the non-Brandon speaker is the NPC we're
	//     conversing with. partner = scripted chat partner when known (only a few
	//     scenes resolve it). new=1 marks the line that opens a fresh conversation.
	//     Helps map NPC talk / turn-in locations. Prefix "APCHAT". ---
	warning("APCHAT scene=%d speaker=%d partner=%d voc=%d new=%d line=\"%s\"",
	        _currentCharacter->sceneId, (int)charNum, (int)chatPartnerNum,
	        vocFile, (int)convoInitialized, chatStr ? chatStr : "");
