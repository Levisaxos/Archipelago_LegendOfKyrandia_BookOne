...
void KyraEngine_LoK::enterNewScene(int sceneId, int facing, int unk1, int unk2, int brandonAlive) {
	...
	// --- AP scrape: dump this scene's item layout (to stderr, prefix "APSCRAPE")
	//     so we can build the (sceneId, slot) -> AP location map. Capture stderr to
	//     a file and walk/teleport through the game once. ---
	{
		const char *sceneName = (_roomTable[sceneId].nameIndex < _roomFilenameTableSize)
		                            ? _roomFilenameTable[_roomTable[sceneId].nameIndex] : "?";
		for (int s = 0; s < 12; ++s) {
			int it = _roomTable[sceneId].itemsTable[s];
			if (it != kItemNone) {
				const char *nm = _itemList ? _itemList[getItemListIndex(it)] : "?";
				warning("APSCRAPE scene=%d name=%s slot=%d item=%d itemname=%s", sceneId, sceneName, s, it, nm);
			}
		}
	}
